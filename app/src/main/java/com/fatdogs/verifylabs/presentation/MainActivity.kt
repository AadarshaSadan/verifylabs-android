        ...
        viewModel.getPostsAPIObserver().observe(this) { resource ->
            when (resource.status) {
                ...
                Status.SUCCESS ->if (resource.data != null) {
                    ...
                    try {
                        val response=Gson().fromJson<PostResponse>(resource.data.toString(), PostResponse::class.java)
                        Log.d("MainActivity", "Posts parsed: ${response.id}")
                        Toast.makeText(this, "Posts parsed: ${response.title}", Toast.LENGTH_LONG).show()
