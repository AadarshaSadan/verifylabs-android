...
@HiltViewModel
class HistoryViewModel @Inject constructor(private val repository: VerificationRepository) :
        ViewModel() {
    ...
    fun deleteHistoryItem(id: Int) {
        viewModelScope.launch { repository.deleteById(id.toLong()) }
    }

    fun purgeHistory() {
        viewModelScope.launch { repository.purgeAll() }
    }

    fun cleanupExpired() {
        viewModelScope.launch {
            // Match iOS cleanup logic if retention days are defined,
            // otherwise using a default (e.g. 30 days) if applicable.
            repository.deleteOlderThan(30)
        }
    }

    suspend fun getStorageSizeKb(): Long {
        return repository.getTotalSizeKb()
    }

    private fun VerificationEntity.toHistoryItem() =
            HistoryItem(
                    id = id.toInt(),
                    title = bandName,
                    type = mediaType,
